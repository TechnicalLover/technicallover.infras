...
az.cmd group create --name $keyVaultRG --location $region

echo "Creating Key Vault . . ."
az.cmd keyvault create --name $keyVaultName --resource-group $keyVaultRG --location $region --enabled-for-deployment


# Due to KeyVault DNS propagation, we have to wait for 10 seconds (see https://github.com/Azure/azure-cli/issues/3348)
while [[ -z "$(az.cmd keyvault show -n $keyVaultName -g $keyVaultRG)" ]]; do
  sleep 10s
...
az.cmd keyvault certificate import --vault-name $keyVaultName --name $keyVaultSecretName --file $certDNSName.pfx --password $certPassword
...
# Create a cert with the specified policy. Use 'az.cmd keyvault certificate get-default-policy' to 
# get the default policy, and then change the 'subject' in the policy to match the FQDN for the cluster.
# az.cmd keyvault certificate create --vault-name $keyVaultName --name $keyVaultSecretName-p @policy.json

# Download the secrete (private key information) associated with the cert
# az.cmd keyvault secret download --vault-name $keyVaultName --name $keyVaultSecretName --encoding base64 --file $certDNSName.pfx
